(() => {
  ...
  const sfx = {
    rollTick() { playNoise(0.045, 0.16, 0, 2200 + Math.random() * 900); },
    move() { playTone(520, 0.1, "triangle", 0.15, 0, 440); },
    capture() {
      playNoise(0.12, 0.22, 0, 700, "lowpass");
      playTone(150, 0.22, "sawtooth", 0.13, 0.03, 65);
    },
    finish() {
    ...
    win() {
      [523.25, 659.25, 783.99, 1046.5].forEach((f, i) => playTone(f, 0.38, "triangle", 0.15, i * 0.12));
    },
    click() { playTone(440, 0.045, "square", 0.07); },
  };
  ...
  function colorsForCount(count) {
    return count === 2 ? ["red", "yellow"] : ["red", "green", "yellow", "blue"];
  }
